...
#                       M. Jin 2024.09
#       rev. 100924 Adding tab menus & separating CLUT/Measure/Test functions
#            102624 Auto RGB gray stepping
#            102724 Store modified or measured data
#                   **** remaining tasks:   data acquisition (Arduino+ADC) for measurements
#                                           save/ recall & plot CLUT csv files
#                                           file select folder function
# ******************************************************************
...
# *********** Tab Menu Controls ***************
tabControl = ttk.Notebook(root, width=int(1200), height=menuHeight)

style = ttk.Style()
style.theme_use('default')
style.configure('TNotebook.Tab', background="gray80")
style.map("TNotebook.Tab", foreground=[("selected", "white")], background = [("selected", "red4")]) # foreground (font) background (tab)

tab1 = Frame(tabControl); tab2 = Frame(tabControl)
tab3 = Frame(tabControl); tab4 = Frame(tabControl)
#tab1.pack_propagate(0); tab2.pack_propagate(0)

# put the main frame into notebook
tabControl.add(tab1, text='CLUT Menu')
tabControl.add(tab2, text='Measure')
tabControl.add(tab3, text ='Verify') 
tabControl.add(tab4, text ='Custom Images')
tabControl.pack(expand = 1, fill ="both") 
tabControl.place(x=10, y=0)

# ++++++++++ Coords for figure canvases within Tabs ++++++++++++++++
...
# ***************** Gamma figure within Tab1 *****************
smallCanvas = Canvas(tab1, width=winWidth, height=winHeight, highlightbackground="black", bg="gray90")
...
rootTable = Canvas(tab1, width=tableWidth, height=tableHeight, highlightbackground="black", bg="gray80")
...
def spline():
    ...
    # splineArray = [[b0,g0,r0],...,[b1023,g1023,r1023]]
...
def grayIncrement():
    global counter
    global color
    pauseT=measDuration.get()
    gStep=gIncrement.get()
    if color<3:
        if color==0:
            b=1; g=0; r=0
        if color ==1:
        ...
        if grayN==256:
            grayN=255
        hexColor=rgb2hex(b*grayN, g*grayN, r*grayN)
        win1.configure(background=hexColor) # set the colour to the next colour generated rootCanvas
        rootCanvas.configure(background=hexColor) # set the colour to the next colour generated
        acquireData()
        rootCanvas.after(pauseT, grayIncrement) # run this function again after 1000ms
        print (color, grayN)
        counter = counter +1
        if grayN==255:
            counter=0; color=color+1
    else:
        n=128
        hexColor=rgb2hex(n, n, n)
        win1.configure(background=hexColor) # set the colour to the next colour generated rootCanvas
        rootCanvas.configure(background=hexColor) # set the colour to the next colour generated
        print ("done")
        counter=0; color=0
...
def autoMeasure():  # timer based auto grayscale (RGB) change + data acquisition
    ...
    beginGray()
    #update_SLM_data()
    tabControl.select(tab1) # switch tab to show acquired data
...
def onTimeInc(event=None):
    measDuration.set(int((measDuration.get())+50))
    if measDuration.get()>1000:
        measDuration.set(50)
        
def onTimeDec(event=None):
    measDuration.set(int((measDuration.get())-50))
    if measDuration.get()<50:
        measDuration.set(1000)
...
class MenuButtons:
    ...
    def __init__(iself, master):        # __init__(className, variable)
        iself.redButton = Button(tab2,text="Red", activebackground='red3', bg='red', fg='white', width=8, height=1, command=iself.rStart)
        iself.greenButton = Button(tab2,text="Green", activebackground='green', bg='green3', fg='white', width=8, height=1, command=iself.gStart)
        iself.blueButton = Button(tab2,text="Blue", activebackground='blue4', bg='blue', fg='white', width=8, height=1, command=iself.bStart)
        iself.whiteButton = Button(tab2,text="White",activebackground='gray80', bg='white', fg='black', width=8, height=1, command=iself.wStart)       
        iself.upButton = Button(tab2,text="Up",activebackground='gray64', bg='gray80', fg='black', width=5, height=1, command=onClickUp) 
        iself.gLevel = Label(tab2, textvariable=counter, height=2, border=0, relief="solid", anchor="e", justify="center", 
                            bg="gray", fg="white", font=('Arial', 14))
        iself.downButton = Button(tab2,text="Down",activebackground='gray64', bg='gray80', fg='black', width=5, height=1, command=onClickDown) 
        iself.incLabel = Label(tab2, text="Increment", width=8, height=1, justify="center")
        iself.keyIndex = Label(tab2, text="Press: R, G, B, W, Up, Down, Left, Right, Esc", width=40, height=1, justify="left")
        
        iself.grayNLabel = Label(tab1, text="Gray", width=8, height=1, justify="center")
        iself.redLabel = Label(tab1, text="Red", width=8, height=1, justify="center")
        iself.greenLabel = Label(tab1, text="Green", width=8, height=1, justify="center")
        iself.blueLabel = Label(tab1, text="Blue", width=8, height=1, justify="center")
        
        iself.plusButton = Button(tab2,text="+",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onClickInc) 
        iself.deltaGray = Label(tab2, textvariable=gIncrement, width=5, height=1, justify="center")
        iself.minusButton = Button(tab2,text="-",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onClickDec) 
        iself.measSLM = Button(tab2,text="Measure SLM",activebackground='gray64', bg='gray80', fg='black', width=12, height=1, command=autoMeasure)
        
        iself.incButton = Button(tab2,text="+",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onTimeInc) 
        iself.deltaTime = Label(tab2, textvariable=measDuration, width=5, height=1, justify="center")
        iself.decButton = Button(tab2,text="-",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onTimeDec) 
        iself.timeLabel = Label(tab2, text="Gray Duration (ms)", width=16, height=1, justify="center")
        ...
        iself.clutLabel = Label(tab1, text="CLUT Depth :", justify="left")
        iself.CLUT10 = Button(tab1,text="10 bit", activebackground='gray64', bg='green4', fg='black', width=12, height=1, command=iself.bit10)
        iself.CLUT12 = Button(tab1,text="12 bit", activebackground='gray64', bg='gray80', fg='black', width=12, height=1, command=iself.bit12)
        
        iself.storeSLM = Button(tab1,text="Store Data",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=store_data)
        iself.readSLMdata = Button(tab1,text="Read Data",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=read_Data)
        iself.genCLUT = Button(tab1,text="Generate CLUT",activebackground='gray64', bg='gray80', fg='black', width=12, height=1, command=generate_CLUT)
        iself.storeCLUT = Button(tab1,text="Store CLUT",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=iself.wStart)
        iself.recallCLUT = Button(tab1,text="Recall CLUT",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=iself.wStart)
        
        iself.clearButton = Button(tab1,text="Clear Data",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=clearData)
        iself.updateButton = Button(tab1,text="Update Curve",activebackground='gray64', bg='gray80', fg='black', width=12, height=1, command=updateSpline)
        
        iself.gammaLabel = Label(tab1, text="CLUT Gamma", width=16, height=1, justify="center", font=('Arial', 11))
        iself.gammaUp = Button(tab1,text="+",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onGammaInc) 
        iself.gammaValue = Label(tab1, textvariable=gammaVal, width=4, height=1, justify="right",font=('Arial', 10))
        iself.gammaDown = Button(tab1,text="-",activebackground='gray64', bg='gray80', fg='black', width=3, height=1, command=onGammaDec) 
        iself.gammaLinear = Button(tab1,text="Linear",activebackground='gray64', bg='gray80', fg='black', height=1, command=onGammaLinear)
        
        
        dy=60; dx0= 120; dx=50; dx1=200; dx2=65; dy2=60; dx3=300
        y0=90
        x1=15; x2=200
        ...
        iself.incButton.place(x=dx3+x2+dx*2+20, y=y1+dy*1.5, width=40, height=bHeight)
        iself.deltaTime.place(x=dx3+x2+dx, y=y1+dy*1.5, height=bHeight)
        iself.decButton.place(x=dx3+x2, y=y1+dy*1.5, width=40, height=bHeight)
        iself.timeLabel.place(x=dx3+x2-20, y=y1+dy*2.25)
