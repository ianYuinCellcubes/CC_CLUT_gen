...
smallCanvas.place(x=50, y=sCanBottom)
# *******************************************************

fig, ax = plt.subplots()
fig, ax = plt.subplots(figsize=(4, 2.8), sharey=True)           
...
class MenuButtons:
    ...
    def __init__(iself, master):        # __init__(className, variable)
        ...
        iself.readSLMdata = Button(root,text="Read Data",activebackground='gray64', bg='gray80', fg='black', width=10, height=1, command=iself.wStart)
        ...
        dy=60; dx=50; dx1=200; dx2=65; dy2=80
        y0=70
        x1=15; x2=250
        y1=70; y2=y1+200; y3= y2+dy2*5+25
        
        iself.button1.place(x=10, y=y0)
        iself.button2.place(x=10, y=y0+dy)
        iself.button3.place(x=10, y=y0+dy*2)
        iself.button4.place(x=10, y=y0+dy*3)
        iself.upButton.place(x=150, y=y1+dy)
        iself.gLevel.place(x=int(canvasWidth/2)+canvasX-20, y=int(canvasHeight/2)+canvasY-30)
       # iself.gLevel.place(x=1000, y=250+dy*1.5)
        iself.downButton.place(x=150, y=y1+dy*2)
        iself.incLabel.place(x=x2+20, y=y1+dy*0.75)
